...
export default function Dashboard() {
  ...
  const t = (text: string) => {
    ...
    // Simulacre de traduction pour l'aperçu
      ...
      <aside className="w-64 bg-white border-r border-gray-200 shadow-sm hidden md:flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          <button 
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === "hall" ? "bg-amber-50 text-amber-700 border-r-4 border-amber-600" : "text-gray-600 hover:bg-gray-50"}`}
          >
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === "restaurants" ? "bg-amber-50 text-amber-700 border-r-4 border-amber-600" : "text-gray-600 hover:bg-gray-50"}`}
          >
            <Store size={18} />
            Mes Restaurants
          </button>
          <button 
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === "editor" ? "bg-amber-50 text-amber-700 border-r-4 border-amber-600" : "text-gray-600 hover:bg-gray-50"}`}
          >
            <Edit size={18} />
            Éditeur de Menu
          </button>
        ...
        <div className="flex-1 p-4 md:p-8 max-w-7xl mx-auto w-full">
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">Bon retour parmi vos menus. Voici l'activité du jour.</p>
                </div>
                ...
                {["today", "7d", "30d", "all"].map((p) => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${
                      period === p ? "bg-amber-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}
                  >
                    {p === "today" && "Aujourd'hui"}
                    {p === "7d" && "7 jours"}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                  <p className="text-sm text-gray-500">Scans - {getPeriodLabel()}</p>
                  <p className="text-3xl font-bold text-amber-700">{scanCount}</p>
                  <p className="text-xs text-green-600 mt-1">↑ 12% vs période précédente</p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                  <p className="text-sm text-gray-500">Restaurants actifs</p>
                  <p className="text-3xl font-bold text-gray-800">4</p>
                  <p className="text-xs text-gray-400 mt-1">Sur 5 inscrits</p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                  {Object.entries(cantonData).map(([c, v]) => (
                    <div key={c} className="bg-amber-50 p-3 rounded-lg text-center">
                      <p className="text-lg font-bold text-amber-800">{v}</p>
                      <p className="text-xs font-medium text-amber-600">{c}</p>
                    </div>
                  ))}
                </div>
                <div className="mt-4 p-3 bg-gray-50 rounded-lg text-sm text-gray-600">
                  <span className="font-medium">📊 Ce que cela signifie pour vous :</span> Les cantons de Vaud et Genève concentrent le plus de scans. Les pics d'activité se situent entre 12h et 13h, puis entre 19h et 20h.
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === "restaurants" && (
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h2 className="text-2xl font-bold text-gray-800">Mes Restaurants</h2>
                  <p className="text-gray-500 text-sm">Gérez vos établissements en un clic.</p>
                </div>
                <button className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th className="px-6 py-3 font-semibold text-gray-600">Nom</th>
                        <th className="px-6 py-3 font-semibold text-gray-600">Canton</th>
                        <th className="px-6 py-3 font-semibold text-gray-600">Statut</th>
                    ...
                    <tbody>
                      {restaurantsData.map((r) => (
                        <tr key={r.id} className="border-b hover:bg-gray-50 transition">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${r.status === "Actif" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                              {r.status}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <button className="text-amber-600 hover:text-amber-800 font-medium flex items-center justify-end gap-1 ml-auto">
          ...
          {/* ===== ONGLET 3 : SUITE ROYALE ===== */}
          {activeTab === "editor" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-800">Éditeur de Menu</h2>
                <p className="text-gray-500 text-sm">Collez le JSON de votre menu et prévisualisez-le en direct.</p>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Colonne gauche : JSON + Color Picker */}
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-medium text-gray-700">Données JSON</label>
                    <textarea 
                      className="w-full h-[400px] p-4 bg-gray-50 border border-gray-300 rounded-xl font-mono text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                      value={jsonInput}
                      onChange={handleJsonChange}
                      spellCheck={false}
                    />
                    <p className="text-xs text-gray-400 mt-1">Collez ici le JSON extrait par l'IA. Chaque catégorie contient une liste de plats avec nom, prix et description.</p>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    />
                    <span className="text-xs text-gray-400">Change la couleur du thème du menu</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-sm font-medium text-gray-700">Aperçu en direct</label>
                      ...
                      {["FR", "DE", "IT", "EN"].map((l) => (
                        <button
                          ...
                          className={`px-2 py-0.5 rounded text-xs font-medium border transition ${
                            lang === l ? "bg-amber-100 text-amber-800 border-amber-300" : "bg-gray-100 text-gray-500 border-gray-200 hover:bg-gray-200"
                          }`}
                        >
                          {l}
                        </button>
                  ...
                  <div 
                    className="border border-gray-200 rounded-xl bg-white p-6 h-[450px] overflow-y-auto shadow-inner"
                    style={{ borderColor: primaryColor }}
                  >
                    {menuPreview && menuPreview.restaurantName ? (
                      <div>
                        <h3 className="text-2xl font-bold border-b pb-2 mb-4" style={{ color: primaryColor, borderColor: primaryColor }}>
                          {menuPreview.restaurantName}
                        </h3>
                        {menuPreview.categories.map((cat, idx) => (
                          <div key={idx} className="mb-4">
                            <h4 className="text-lg font-semibold text-gray-700 border-l-4 pl-3 mb-2" style={{ borderColor: primaryColor }}>
                              {t(cat.name)}
                            </h4>
                            {cat.items.map((item, i) => (
                              <div key={i} className="flex justify-between items-start py-1.5 border-b border-dashed border-gray-100">
                                <div>
                                  ...
                                  {item.description && <p className="text-xs text-gray-400">{t(item.description)}</p>}
                                </div>
                        ...
                        <div className="mt-4 text-center text-xs text-gray-400 border-t pt-2">
                          🥄 Service à table · Menu digital CARTIS
                        </div>
                      </div>
                    ) : (
                      <div className="text-gray-400 flex items-center justify-center h-full text-center">
                        <div>
                          ...
                          <p className="text-sm">Vérifiez le format de votre JSON</p>
                        </div>
                      </div>
                    )}
                  </div>
